...
function cleanName(text) {
  return String(text || "")
    .replace(/\b(olsun|olacak|yap|aç|ac|oluştur|olustur|create|make|inside|under|ve|and)\b/gi, " ")
    .replace(/\b(kategori|category|kanal|channel|sesli|voice|text|metin)\b/gi, " ")
    .replace(/\s+/g, " ")
    .trim()
}
...
function detectModerationPlan(message, text, language) {
  ...
  const isBan = value.includes("banla") || value.startsWith("ban ") || value.includes(" yasakla")
  const isKick = value.includes("kick") || value.includes("at sunucudan") || value.includes("cikar")
  const isUnmute = value.includes("unmute") || value.includes("mute kaldir") || value.includes("timeout kaldir")
  const isMute = !isUnmute && (value.includes("mute") || value.includes("sustur") || value.includes("timeout"))
    ...
    input: [
      {
        ...
        content: [
          {
            type: "input_text",
            text: "You parse Discord management commands in Turkish and English. Return only valid JSON. Understand category creation, channel creation, rename, delete, move, voice/text channel requests, and topic changes."
          }
        ]
      },
      {
...
async function detectManagementPlan(openai, model, guild, member, text, language) {
  ...
  const value = normalize(text)
  const signalWords = [
    "kategori", "kanal", "channel", "category",
    "olustur", "sil", "delete", "rename", "tasi", "move",
    "topic", "aciklama", "voice", "sesli"
  ]
